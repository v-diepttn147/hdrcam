...
import com.hdrcam.camera.utils.gles.program.Program;
...
public final class GLESUtils {
    ...
    public static int loadTexture(ByteBuffer rgbaBuffer, int width, int height) {
        rgbaBuffer.rewind();
        final int[] textureHandle = new int[1];
        GLES20.glGenTextures(1, textureHandle, 0);
        if (textureHandle[0] == 0) {
    ...
    public static int[] loadTextureNV(ByteBuffer yBuffer, ByteBuffer uvBuffer,
                                      int width_yuv,
                                      int height_yuv) {
        yBuffer.rewind();
        uvBuffer.rewind();
        int nTex = 2;
        int[] textureId_yuv = new int[nTex];
//        int minMode = GLES20.GL_LINEAR;

        GLES20.glGenTextures(nTex, textureId_yuv, 0);
        ...
        GLES20.glTexImage2D(GLES20.GL_TEXTURE_2D, 0, GLES20.GL_LUMINANCE, width_yuv, height_yuv, 0, GLES20.GL_LUMINANCE, GLES20.GL_UNSIGNED_BYTE, yBuffer);
        ...
        GLES20.glTexImage2D(GLES20.GL_TEXTURE_2D, 0, GLES20.GL_LUMINANCE_ALPHA,
                width_yuv / 2, height_yuv / 2, 0, GLES20.GL_LUMINANCE_ALPHA, GLES20.GL_UNSIGNED_BYTE,
                uvBuffer);
    ...
    /* YUV420 texture*/
    public static int[] loadTextureYUV420(ByteBuffer y, ByteBuffer u, ByteBuffer v,
                                          int width_yuv,
                                          int height_yuv) {
        y.rewind();
        u.rewind();
        ...
        GLES20.glTexParameteri(GLES20.GL_TEXTURE_2D, GLES20.GL_TEXTURE_WRAP_S, GLES20.GL_REPEAT);
        GLES20.glTexParameteri(GLES20.GL_TEXTURE_2D, GLES20.GL_TEXTURE_WRAP_T, GLES20.GL_REPEAT);
        GLES20.glTexParameteri(GLES20.GL_TEXTURE_2D, GLES20.GL_TEXTURE_MIN_FILTER, minMode);
        GLES20.glTexParameteri(GLES20.GL_TEXTURE_2D, GLES20.GL_TEXTURE_MAG_FILTER, GLES20.GL_LINEAR);
    ...
    public static int createImageTexture(ByteBuffer pixelBuff, int width, int height) {
        if (pixelBuff == null) {
            return 0;
        }
        int[] textureHandles = new int[1];
        ...
        //GLUtils.texImage2D(GLES20.GL_TEXTURE_2D, 0, bmp, 0);
        GLES20.glTexImage2D(GLES20.GL_TEXTURE_2D, 0, GLES20.GL_RGBA, width, height, 0,
                GLES20.GL_RGBA, GLES20.GL_BYTE, pixelBuff);
        // 生成 MIP 贴图
        GLES20.glGenerateMipmap(GLES20.GL_TEXTURE_2D);
        GLES20.glBindTexture(GLES20.GL_TEXTURE_2D, 0);
        return textureHandles[0];
    ...
    public static float[] setPhotoPreviewMVP(float viewWidth, float viewHeight,
                                             float textureWidth, float textureHeight) {
        //float scale = viewWidth * textureHeight / viewHeight / textureWidth;
        float[] mvp = new float[16];


        float targetAspectRatio = textureHeight / textureWidth;

        float backAspectRatio = viewWidth / viewHeight;

        float ratio = backAspectRatio / targetAspectRatio;

        Matrix.setIdentityM(mvp, 0);
        //Matrix.scaleM(mvp,0,-1,0,0);
        Matrix.orthoM(mvp, 0, -ratio, ratio, -1.0f, 1.0f, -1.0f, 1.0f);
        Matrix.rotateM(mvp, 0, 180, 0.0f, 1.0f, 0.0f);

        //Matrix.scaleM(mvp, 0, scale > 1 ? (1F / scale) : 1F, scale > 1 ? 1F : scale, 1F);
        return mvp;
    }

    public static float[] setBackCameraPreviewMVP(float viewWidth, float viewHeight,
                                                  float textureWidth, float textureHeight) {
        //float scale = viewWidth * textureHeight / viewHeight / textureWidth;
        float[] mvp = new float[16];
        ...
        //Matrix.scaleM(mvp,0,-1,0,0);
        Matrix.orthoM(mvp, 0, -ratio, ratio, -1.0f, 1.0f, -1.0f, 1.0f);
        Matrix.rotateM(mvp, 0, -90, 0.0f, 0.0f, 1.0f);
        Matrix.rotateM(mvp, 0, 180, 0.0f, 1.0f, 0.0f);
    ...
    public static float[] setFrontCameraPreviewMVP(float viewWidth, float viewHeight,
                                                   float textureWidth, float textureHeight) {
        //float scale = viewWidth * textureHeight / viewHeight / textureWidth;
        float[] mvp = new float[16];
    ...
    public static Bitmap glReadCameraBitmap(int[] textureId, float[] texMatrix, float[] mvpMatrix,
                                            final int texWidth,
                                            final int texHeight, Program textureProgram) {
        int[] fboId = new int[1];
        int[] fboTex = new int[1];
        createFBO(fboTex, fboId, texWidth, texHeight);
...
/*
        ...
        buf.rewind();
        Bitmap glBMP = Bitmap.createBitmap(texWidth, texHeight, Bitmap.Config.ARGB_8888);
        glBMP.copyPixelsFromBuffer(buf);
